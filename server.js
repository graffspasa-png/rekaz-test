...
async function getProds() {
  ...
  // 2. Gift products from /api/public/products?type=3
  // Already included in the type=3 fetch below
...
app.post("/gift/purchase", async (req, res) => {
  ...
  try {
    ...
    // ── Step 1: Resolve priceId ──
    // Confirmed priceIds from GET /api/public/products (Gift CARD product)
    ...
    if (!giftPriceId) {
      console.log("[Gift] No priceId — pending_review");
    } else {
      ...
      // ── Step 2: Find or create customer (to get customerId) ──
      let customerId = null;
      try {
      ...
      // ── Step 3: POST /api/public/orders ──
      const orderPayload = {
        items: [{ priceId: giftPriceId, quantity: 1 }],
        invoiceNote: note,
        branchId: BRANCH_ID
      };
      if (customerId) orderPayload.customerId = customerId;
      else orderPayload.customerDetails = { name: fromName, mobileNumber: fromMobile, type: 0 };
      ...
      try {
        ...
        console.log("[Gift] POST /orders →", r.status, rawText.slice(0, 400));
        if (r.ok) {
          const d  = r.json();
          ...
          console.log("[Gift] /orders success → payUrl:", payUrl);
        }
      ...
      if (!payUrl) {
        try {
          ...
          const rawText2 = r2.text;
          console.log("[Gift] POST /subscriptions fallback →", r2.status, rawText2.slice(0, 400));
          if (r2.ok) {
            const d2  = r2.json();
            ...
            payUrl    = pp2
              ...
              : (invoiceId ? `${REKAZ_BASE}/i/${invoiceId}` : null);
            console.log("[Gift] /subscriptions → payUrl:", payUrl);
          }
...
app.post("/api/purchase-gift", async (req, res) => {
  ...
  try {
    ...
    for (const customerType of customerTypes) {
      ...
      console.log("[api/purchase-gift] trying type:", customerType, "priceId:", GIFT_PRICE_ID);
      ...
      try {
        const r = await rekazFetch(`${REKAZ_API}/subscriptions`, {
          method: "POST",
          body: JSON.stringify({
            customerDetails: { name: senderName, mobileNumber: mobile, type: customerType },
            branchId: BRANCH_ID,
            invoiceNote: note,
            items: [{ priceId: GIFT_PRICE_ID, quantity: 1 }]
          })
        });
        console.log("[api/purchase-gift] /subscriptions type:" + customerType + ":", r.status, r.text.slice(0, 300));
        if (r.ok) {
          const d = r.json();
      ...
      try {
        ...
        console.log("[api/purchase-gift] /reservations/bulk type:" + customerType + ":", r2.status, r2.text.slice(0, 300));
        if (r2.ok) {
          const d2 = r2.json();
          invoiceId   = d2.invoiceId || d2.id || invoiceId || null;
          const pp2   = d2.paymentLink || d2.payUrl || d2.link || "";
...
app.get("/gift-product-info", async (req, res) => {
  rCacheTime = 0;
  const data = await getProds();
